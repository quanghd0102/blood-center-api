...
const mainWebUrl = process.env.WEB_URL || '';
...
class AuthService {
  async login(body) {
    try {
      ...
      const isCorrectPassword = await PasswordUtils.compare(body.password, user.password);
  ...
  async loginFacebook(request) {
    try {
      ...
      console.log(profile);
      if (!profile.id) {
        return Boom.badRequest('This access token is not registered by the application');
      }
