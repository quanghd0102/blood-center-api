...
function parsingAddress(result, language) {
  ...
  const formatted_address =
    lastIndex > -1 ? result.formatted_address.substring(0, lastIndex) : result.formatted_address;
  if (language === 'vi') {
    components.formatted_address =
      formatted_address && formatted_address.replace('Unnamed Road', 'Đường chưa đặt tên');
  } else {
  ...
  each(result.address_components, component => {
    ...
    if (intersection(component.types, ['administrative_area_level_3']).length) {
      components.administrative_area_level_3 = component.short_name.replace(/(TT. )|(tt. )/g, '');
    }
    ...
    if (intersection(component.types, ['locality']).length) {
      components.locality = component.long_name.replace(/(tp. )|(tt. )|(h. )|(Tx. )|(tx. )/g, '');
    }
  ...
  if (components.postcode) {
    components.formatted_address = components.formatted_address.replace(components.postcode, '');
  }
  ...
  if (!components.administrative_area_level_3) {
    components.administrative_area_level_3 = components.formatted_address.split(', ')[1];
  }
  return components;
}
