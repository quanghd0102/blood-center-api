...
const mailgunOptions = {
  auth: {
    api_key: process.env.EMAIL_API_KEY || 'sfgsfdg',
...
async function sendInfoNewAccount(receiverEmail, info) {
  try {
    const mailOptions = {
      ...
      html: `<p>Your new account is all set to go! You can access it using the following credentials:</p><p>Full Name: ${info.fullName}<br />Email: ${info.email}<br />Password: ${info.password}</p>`
    };
    const result = await emailClient.sendMail(mailOptions);
    return result;
  } catch (err) {
